              ...
              <div className="wiring-admin-brand" key={`${categoryName}-${brandName}`}>
                ...
                {brandParts.map((part) => (
                  <AccordionCard
                    ...
                    imageUrl={part.imageUrl}
                    title={part.name}
                    meta={part.availability || "Available"}
...
function AutoSliderBannerManager({ banners, products, runAction, busy }) {
  const empty = { imageUrl: "", title: "", alt: "", displayOrder: 0, isActive: true };
  const [form, setForm] = useState(empty);
  const [editingId, setEditingId] = useState("");
  ...
  const save = () => runAction(async () => {
    await apiFetch(editingId ? `/admin/auto-slider-banners/${editingId}` : "/admin/auto-slider-banners", {
      method: editingId ? "PUT" : "POST", body: JSON.stringify({ ...form, displayOrder: Number(form.displayOrder || 0) }),
    });
    reset();
  }, editingId ? "Hero banner updated" : "Hero banner added");
  const edit = (banner) => { setEditingId(banner._id || banner.id); setForm({ imageUrl: banner.imageUrl || "", title: banner.title || "", alt: banner.alt || "", displayOrder: banner.displayOrder || 0, isActive: banner.isActive !== false }); };
  ...
  return <div className="manager-grid">
    <section className="editor glass-panel">
      ...
      <p className="muted">Custom banners and products marked “Show in Hero Slider” rotate automatically on the home page.</p>
      <ImageField value={form.imageUrl} onChange={(imageUrl) => setForm({ ...form, imageUrl })} />
    ...
    <section className="list-panel glass-panel">
      ...
      {banners.map((banner) => <div className="list-item" key={banner._id || banner.id}><img className="thumb" src={banner.imageUrl} alt={banner.alt || banner.title || "Hero banner"} /><div><strong>{banner.title || "Untitled banner"}</strong><span>Order {banner.displayOrder || 0} · {banner.isActive ? "Enabled" : "Disabled"}</span></div><div className="button-row"><button type="button" onClick={() => edit(banner)}>Edit</button><button className="danger" type="button" onClick={() => remove(banner)}>Delete</button></div></div>)}
      ...
      {products.map((product) => <div className="list-item" key={product._id || product.id}><img className="thumb" src={product.imageUrl} alt={product.name} /><div><strong>{product.name}</strong><span>{product.isActive ? "Active" : "Hidden on public site"}</span></div></div>)}
    </section>
  </div>;
}

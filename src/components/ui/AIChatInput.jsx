...
const WAVE_BAR_COUNT_DESKTOP = 48;
const WAVE_BAR_COUNT_MOBILE = 24;
const WAVE_MIN = 0.08;
...
/** Mobile browsers often can't share the mic between getUserMedia + SpeechRecognition. */
function isMobileVoiceEnvironment() {
  ...
  return Boolean(coarse || mobileUa || (narrow && "ontouchstart" in window));
}
...
export function AIChatInput({
  ...
  const mobileVoice = useRef(isMobileVoiceEnvironment());
  ...
  const paintWaveBars = () => {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const nextHeight = `${Math.max(8, level * 100)}%`;
      const nextOpacity = String(fade);
      if (bar.style.height !== nextHeight) bar.style.height = nextHeight;
      if (bar.style.opacity !== nextOpacity) bar.style.opacity = nextOpacity;
    }
  };
  ...
  const stopVoice = ({ userInitiated = false } = {}) => {
    ...
    // Commit any leftover interim speech before tearing down.
    const leftover = String(interimTextRef.current || "").trim();
    if (leftover) {
      const nextBase = baseValueRef.current
        ? `${baseValueRef.current} ${leftover}`.trim()
        : leftover;
  ...
  const startAudioMeters = async () => {
    // Skip live AudioContext meter on low-power devices — static/fallback wave is enough.
    if (lowPower.current || !navigator.mediaDevices?.getUserMedia) return;
  ...
  useEffect(() => {
    ...
    const fallback = window.setInterval(() => {
      ...
      history.push(sample);
      paintWaveBars();
    }, lowPower.current || mobileVoice.current ? 100 : 70);
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      ...
      if (!wantListeningRef.current || !analyserRef.current || rafRef.current) return;
      const tick = () => {
        if (!analyserRef.current || !wantListeningRef.current) return;
        if (document.visibilityState === "hidden") {
          rafRef.current = 0;
        ...
        // Push a quiet sample so the loop stays alive until startAudioMeters tick resumes
        // via existing analyser — re-enter by scheduling paint-only idle frames.
        paintWaveBars();
        rafRef.current = window.requestAnimationFrame(tick);
      };
      rafRef.current = window.requestAnimationFrame(tick);
  ...
  const handleSubmit = (event) => {
    ...
    if (listening) {
      const leftover = String(interimTextRef.current || "").trim();
      nextValue = [baseValueRef.current, leftover].filter(Boolean).join(" ").trim() || nextValue;
      interimTextRef.current = "";
    ...
    if (!nextValue && !hasAttachments) return;
    onSubmit?.(nextValue);
  };
  ...
  const bindRecognitionHandlers = (recognition) => {
    ...
    recognition.onerror = (event) => {
      ...
      if (errorType === "audio-capture") {
        setVoiceError("Microphone unavailable. Close other apps using the mic and try again.");
        stopVoice({ userInitiated: true });
        return;
      }
      if (errorType) {
    ...
    recognition.onend = () => {
      // Flush interim text when a mobile (non-continuous) session ends.
      const leftover = String(interimTextRef.current || "").trim();
      if (leftover) {
        const nextBase = baseValueRef.current
          ? `${baseValueRef.current} ${leftover}`.trim()
      ...
      // Browsers often end recognition after a pause — restart until user stops.
      ...
      const restartDelay = mobileVoice.current ? 280 : 120;
  ...
  const toggleVoice = async () => {
    ...
    if (!SpeechRecognition) {
      setVoiceError("Voice typing is not supported in this browser. Try Chrome on Android, or Edge.");
      return;
    }
    ...
    // continuous:true often breaks transcription on mobile Chrome / Safari.
    recognition.continuous = !mobileVoice.current;
    recognition.maxAlternatives = 1;
    recognitionRef.current = recognition;
    bindRecognitionHandlers(recognition);

    try {
      // Desktop: live wave via getUserMedia. Mobile: skip — exclusive mic lock
      // blocks SpeechRecognition from hearing the user.
      if (!mobileVoice.current) {
        await startAudioMeters();
      }
      setListening(true);
      recognition.start();
    } catch (_error) {
      setVoiceError("Unable to start voice typing right now. Check mic permission and try again.");
      stopVoice({ userInitiated: true });
    }
  };
  ...
  const composerHeight = expanded
    ? EXPANDED_BASE_HEIGHT + extraTextHeight + (listening ? LISTENING_WAVE_EXTRA : 0)
    : COLLAPSED_HEIGHT;
      ...
      <div className="pulse-ai-input-inner">
        <div className="pulse-ai-input-row">
          ...
          <div className={`pulse-ai-field ${listening ? "is-listening" : ""}`}>
            {listening ? (
              <div className="pulse-voice-wave" aria-hidden="true">
                <div className="pulse-voice-wave-track">

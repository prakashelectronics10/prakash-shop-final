...
/** Offset order so both rows show different images at the same moment. */
function offsetGalleryItems(items) {
  if (items.length <= 1) return items.map((item, index) => ({ item, index }));
  const offset = Math.max(1, Math.ceil(items.length / 2));
  return [
    ...items.slice(offset).map((item, i) => ({ item, index: offset + i })),
    ...items.slice(0, offset).map((item, i) => ({ item, index: i })),
  ];
}
...
export function Gallery({ sectionId = "gallery" }) {
  ...
  const bottomSource = useMemo(() => offsetGalleryItems(items), [items]);

  const topLoopItems = useMemo(
    () => buildLoopItems(topSource, loopCopies, "top"),
    [topSource, loopCopies],
  );

  const bottomLoopItems = useMemo(
    () => buildLoopItems(bottomSource, loopCopies, "bot"),
    [bottomSource, loopCopies],
  );

  const durationSec = Math.max(24, items.length * 5);
  // Slightly different speed so rows don't look locked in sync.
  const reverseDurationSec = Math.max(26, Math.round(durationSec * 1.08));
  ...
  return (
    <section id={sectionId || undefined} className="site-section relative overflow-hidden">
      ...
      <div className="gallery-marquee-stack mt-10">
        ...
        />
        <GalleryMarqueeRow
          loopItems={bottomLoopItems}
          direction="ltr"
          durationSec={reverseDurationSec}
          loopCopies={loopCopies}
          forcePaused={lightboxOpen}
          onOpen={setActive}
          eagerCount={0}

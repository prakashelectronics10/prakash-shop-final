...
export function Hero() {
  ...
  return (
    <section id="home" className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
      ...
      <div className="pointer-events-none absolute -top-32 left-1/3 h-[500px] w-[500px] rounded-full bg-gradient-primary opacity-30 blur-3xl animate-glow-pulse" />
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-4 lg:grid-cols-2 lg:gap-8 lg:items-center">
        ...
        <div className="hero-media-card relative mx-auto w-full max-w-lg">
          <div className="absolute -inset-6 rounded-[2rem] bg-gradient-primary opacity-40 blur-2xl animate-glow-pulse" />
          <div className="relative overflow-hidden rounded-3xl glass-strong border-glow shadow-elegant">
            <OptimizedImage
              src={heroData.image?.url}
              alt={heroData.image?.alt || heroData.title}
              width={1280}
              height={1280}
            ...
            {floatingBadges[0] && (
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between rounded-2xl glass-strong p-3">
                <div>
                  <div className="text-xs text-muted-foreground">{floatingBadges[0].label}</div>
                  <div className="font-display text-lg font-bold">{floatingBadges[0].value}</div>
                </div>
                <div className="rounded-xl bg-gradient-primary px-3 py-2 text-xs font-semibold text-primary-foreground shadow-glow">
                  Live now
                </div>
              </div>
            )}
          ...
          {floatingBadges.slice(0, 2).map((badge, index) => (
            <div
              key={`${badge.label}-${index}`}
              className={`hero-floating-badge absolute hidden rounded-2xl glass-strong px-4 py-3 shadow-card md:block ${
                index === 0 ? "-left-6 top-12" : "-right-4 bottom-24"
              }`}
              style={{ animationDuration: index === 0 ? "5s" : "6s", animationDirection: index === 0 ? "normal" : "reverse" }}
            >
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground">
                {badge.label}
              </div>
              <div className="font-display text-xl font-bold text-gradient">{badge.value}</div>
            </div>
          ))}
        </div>
      </div>

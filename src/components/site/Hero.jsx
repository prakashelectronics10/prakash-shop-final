...
const fallbackHero = {
  ...
  title: "Repair service, Wiring services, and",
  highlight: "Electronics products",
  titleSuffix: "Sales and Service",
  description:
  ...
  image: {
    url: "/seed-assets/optimized/hero-technician-960.webp",
...
function HeroImageSlider({ slides, fallbackImage }) {
  ...
  const activeLink = normalizeSlideLink(activeSlide?.link);
  ...
  return (
    <div
      className="hero-media-card relative mx-auto w-full max-w-lg"
      ...
      style={swipe.style}
    >
      <div className="absolute -inset-6 hidden rounded-[2rem] bg-gradient-primary opacity-35 blur-2xl md:block" />
      <div className="relative aspect-[4/3] overflow-hidden rounded-3xl glass-strong border-glow shadow-elegant">
        ...
        {activeLink ? (
          <a
            ...
            aria-label={activeSlide?.title ? `Open ${activeSlide.title}` : "Open banner link"}
      ...
      <ProgressSliderDots
        ...
        onChange={goToSlide}
        intervalMs={4500}
        className="hero-progress-dots"
        ariaLabel="Hero image slides"
      />
    </div>
  );
}
...
export function Hero() {
  ...
  return (
    <section id="home" className="relative overflow-hidden pt-32 pb-20 md:pt-40 md:pb-28">
      ...
      <div className="pointer-events-none absolute -top-32 left-1/3 hidden h-[500px] w-[500px] rounded-full bg-gradient-primary opacity-25 blur-3xl md:block" />
      <div className="pointer-events-none absolute bottom-0 right-0 hidden h-[400px] w-[400px] rounded-full bg-accent/20 blur-3xl md:block" />
      ...
      <div className="relative mx-auto grid max-w-7xl gap-12 px-4 lg:grid-cols-2 lg:gap-8 lg:items-center">
        <div className="animate-fade-up">
          ...
          <h1 className="mt-6 font-display text-4xl font-bold leading-[1.05] tracking-tight sm:text-5xl md:text-6xl lg:text-7xl">
            Repair service, Wiring services, and <span className="text-gradient">Electronics products</span>
          </h1>

          <p className="mt-6 max-w-xl text-base text-muted-foreground sm:text-lg">
            We provides Repairing services, home wiring services and also sells electronics products like Ceiling fan, All wiring accessories, Home appliances, etc.
          </p>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-6 text-sm text-muted-foreground">
            {trustBadges.map((badge, index) => {
              const Icon = badge.iconName === "Star" || /star/i.test(String(badge.iconName || ""))
                ? Star
                : ShieldCheck;
              const isRating = badge.iconName === "Star" || /star/i.test(String(badge.iconName || ""));
              return (

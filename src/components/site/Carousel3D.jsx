...
export const Carousel3D = memo(function Carousel3D({ sectionId = "featured-repairs" }) {
  ...
  useEffect(() => {
    if (!total || prefersReducedMotion) return undefined;
    const tick = () => {
        ...
        <div className="relative mt-16 flex h-[440px] items-center justify-center [perspective:1400px]">
          {cards.map((card, i) => {
            ...
            return (
              ...
              >
                <div
                  className={`relative cursor-pointer overflow-hidden rounded-3xl border-glow p-8 shadow-elegant bg-card/85 backdrop-blur-xl select-none ${
                    rel === 0 ? "shadow-glow" : ""
                  }`}
                >

...
function fcmMessageForToken(token, notification) {
  ...
  const image = String(notification.image || data.image || notification.metadata?.image || "").trim().replace(/^http:\/\//i, "https://");
...
async function sendPushToAdmins(adminIds, notification) {
  ...
  try {
    const [fcmTickets, expoTickets] = await Promise.all([
      fcmMessages.length ? sendFcmMessages(fcmMessages) : Promise.resolve([]),
      expoMessages.length ? sendExpoPushMessages(expoMessages) : Promise.resolve([]),
    ]);

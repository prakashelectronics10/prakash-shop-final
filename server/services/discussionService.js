...
function serializeMessage(message) {
  ...
  return {
    ...
    relatedInvoice: idString(message.relatedInvoice) || null,
    isDeleted: deleted,
...
function normalizeMessagePayload(payload = {}) {
  ...
  const type = attachments.length
    ? attachments[0].type
    : ["text", "image", "file", "document", "pdf", "system"].includes(payload.type)
      ? payload.type
      : "text";
...
async function createMessage(admin, payload = {}) {
  ...
  if (!normalized.message && !normalized.attachments.length) {

...
function serializePin(pin) {
  const admin = pin?.admin;
  return {
    admin: typeof admin === "object" ? serializeAdmin(admin) : { id: idString(admin), _id: idString(admin) },
    adminId: idString(admin),
    scope: pin?.scope === "global" ? "global" : "private",
...
function serializeMessage(message) {
  ...
  return {
    ...
    pins: deleted ? [] : (message.pins || []).map(serializePin).filter((pin) => pin.adminId),
...
function disabledSenderSet(room) {
  return new Set((room?.sendDisabledFor || []).map((entry) => idString(entry.admin)).filter(Boolean));
}

function canSendMessagesInRoom(admin, room) {
  if (!canUseDiscussion(admin)) return false;
  if (isMainAdmin(admin)) return true;
  return !disabledSenderSet(room).has(idString(admin));
}

function requireDiscussionSendAccess(admin, room) {
  requireDiscussionAccess(admin);
  if (!canSendMessagesInRoom(admin, room)) {
    throw new AppError("Main Admin disabled your discussion message sending. You can still read and react.", 403);
  }
}

function serializeDiscussionSettings(room, participants, viewer) {
  const disabled = disabledSenderSet(room);
  const admins = (participants || []).map((admin) => {
    const serialized = serializeAdmin(admin);
    const adminId = idString(admin);
    const mainAdmin = isMainAdmin(admin);
    return {
      ...serialized,
      canSendMessages: mainAdmin || !disabled.has(adminId),
      sendingDisabled: !mainAdmin && disabled.has(adminId),
      canEditSending: !mainAdmin,
    };
  });
  return {
    room: {
      id: idString(room),
      _id: idString(room),
      name: room?.name || "Admin Discussion",
      type: room?.type || "global",
    },
    viewer: {
      admin: serializeAdmin(viewer),
      isMainAdmin: isMainAdmin(viewer),
      canSendMessages: canSendMessagesInRoom(viewer, room),
    },
    sendDisabledAdminIds: [...disabled],
    admins,
  };
}

async function discussionSettings(admin) {
  requireDiscussionAccess(admin);
  const { room, participants } = await getGlobalRoom();
  return serializeDiscussionSettings(room, participants, admin);
}

async function updateDiscussionSendPermission(admin, targetAdminId, canSend) {
  requireDiscussionAccess(admin);
  if (!isMainAdmin(admin)) throw new AppError("Only Main Admin can update discussion sending permissions", 403);
  if (!mongoose.Types.ObjectId.isValid(targetAdminId)) throw new AppError("Invalid admin id", 400);

  const { room, participants } = await getGlobalRoom();
  const target = (participants || []).find((participant) => idString(participant) === idString(targetAdminId));
  if (!target) throw new AppError("Admin is not available in discussion", 404);
  if (isMainAdmin(target)) throw new AppError("Main Admin message sending cannot be disabled", 400);

  if (canSend) {
    await DiscussionRoom.updateOne(
      { _id: room._id },
      { $pull: { sendDisabledFor: { admin: target._id } } },
    );
  } else {
    await DiscussionRoom.updateOne(
      { _id: room._id, "sendDisabledFor.admin": { $ne: target._id } },
      { $push: { sendDisabledFor: { admin: target._id, disabledBy: admin._id, disabledAt: new Date() } } },
    );
  }

  const next = await DiscussionRoom.findById(room._id).lean();
  return serializeDiscussionSettings(next || room, participants, admin);
}
...
async function populateMessage(query) {
  const message = await query
    ...
    .populate("pins.admin", ADMIN_PUBLIC_FIELDS)
...
async function listMessages(admin, query = {}) {
  ...
  return {
    ...
    room: {
      ...
      name: room.name,
      type: room.type,
      sendDisabledAdminIds: [...disabledSenderSet(room)],
      viewerCanSendMessages: canSendMessagesInRoom(admin, room),
      viewerIsMainAdmin: isMainAdmin(admin),
    },
    permissions: serializeDiscussionSettings(room, participants, admin),
...
async function createMessage(admin, payload = {}) {
  requireDiscussionAccess(admin);
  const { room, participants } = await getGlobalRoom();
  requireDiscussionSendAccess(admin, room);
...
async function pinMessage(admin, messageId, scope = "private") {
  requireDiscussionAccess(admin);
  if (!mongoose.Types.ObjectId.isValid(messageId)) throw new AppError("Invalid message id", 400);
  const pinScope = scope === "global" ? "global" : "private";
  if (pinScope === "global" && !isMainAdmin(admin)) {
    throw new AppError("Only Main Admin can pin a message for everyone", 403);
  }
  ...
  const adminId = idString(admin);
  const existing = (message.pins || []).find((pin) =>
    pin.scope === pinScope && (pinScope === "global" || idString(pin.admin) === adminId));

  if (existing) {
    existing.admin = admin._id;
    existing.pinnedAt = new Date();
  } else {
    message.pins.push({ admin: admin._id, scope: pinScope, pinnedAt: new Date() });
  }
  message.markModified("pins");
  await message.save();
...
async function unpinMessage(admin, messageId, scope = "private") {
  requireDiscussionAccess(admin);
  if (!mongoose.Types.ObjectId.isValid(messageId)) throw new AppError("Invalid message id", 400);
  const pinScope = scope === "global" ? "global" : "private";
  if (pinScope === "global" && !isMainAdmin(admin)) {
    throw new AppError("Only Main Admin can unpin a message for everyone", 403);
  }

  const message = await DiscussionMessage.findById(messageId);
  if (!message) throw new AppError("Message not found", 404);
  const adminId = idString(admin);
  message.pins = (message.pins || []).filter((pin) => {
    if (pin.scope !== pinScope) return true;
    if (pinScope === "global") return false;
    return idString(pin.admin) !== adminId;
  });
  message.markModified("pins");
  await message.save();

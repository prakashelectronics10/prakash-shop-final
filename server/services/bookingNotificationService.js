...
function renderAdminBookingEmail({ booking, logoUrl }) {
  ...
  return {
    ...
    text: [
      ...
      `Pincode: ${booking.pincode || "Not provided"}`,
      `Landmark: ${booking.landmark || "Not provided"}`,
                ...
                <tr>
                  <td style="padding:26px 28px;">
                    ...
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:18px;">
                      ...
                      ${row("Landmark", booking.landmark || "Not provided")}
...
async function notifyMobileBookingImmediately(bookingOrId) {
  const bookingId = String(bookingOrId?._id || bookingOrId || "");
  if (!bookingId) return;
  const booking = bookingOrId?._id ? bookingOrId : await Booking.findById(bookingId);
  if (!booking) return;
...
function enqueueBookingNotification(bookingOrId) {
  ...
  notifyMobileBookingImmediately(bookingOrId).catch((error) => {
    console.error("Immediate mobile booking notification failed:", {
      bookingId,
      error: error.message,
    });
  });

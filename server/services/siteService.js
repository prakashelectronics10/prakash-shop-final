...
async function getSitePayload() {
  ...
  try {
    docs = await Promise.all([
      ...
      AutoSliderBanner.find({ isActive: true }).select("imageUrl title alt displayOrder").sort({ displayOrder: 1, createdAt: -1 }).lean(),
      ShopProduct.find({ isActive: true, showInHeroSlider: true }).select("name imageUrl shortDescription displayOrder").sort({ displayOrder: 1, name: 1 }).lean(),
    ]);
  } catch (error) {
  ...
  const payload = {
    ...
    heroSlider: [
      ...bannerDocs.map((banner) => ({ id: String(banner._id), imageUrl: banner.imageUrl, title: banner.title, alt: banner.alt || banner.title, displayOrder: banner.displayOrder, source: "banner" })),
      ...heroProductDocs.filter((product) => product.imageUrl).map((product) => ({ id: String(product._id), imageUrl: product.imageUrl, title: product.name, alt: product.name, description: product.shortDescription, displayOrder: product.displayOrder, source: "product" })),
    ],

...
function adminAvatarUrl(admin = {}) {
  return safeImageUrl(admin.avatarUrl || admin.avatar || admin.imageUrl || admin.profileImage || admin.photoUrl || "");
}

function truncatePushBody(value, limit = 360) {
  const text = String(value || "").replace(/[ \t]+/g, " ").trim();
  return text.length > limit ? `${text.slice(0, limit - 3)}...` : text;
}

function discussionPushMeta(serializedMessage) {
  const invoiceId = serializedMessage.type === "invoice" ? idString(serializedMessage.relatedInvoice || serializedMessage.invoice) : "";
  const invoiceNumber = serializedMessage.type === "invoice" ? serializedMessage.invoice?.invoiceNumber || "" : "";
  const image = discussionImageUrl(serializedMessage);
  return {
    invoiceId,
    invoiceNumber,
    image,
    kind: serializedMessage.type === "invoice" ? "invoice" : undefined,
  };
}

function groupedDiscussionPushBody(messages) {
  if (messages.length <= 1) return discussionPushBodyText(messages[0]);
  const previews = messages.slice(-4).map((message) => `• ${discussionPushBodyText(message)}`);
  return truncatePushBody(`${messages.length} new messages\n${previews.join("\n")}`, 420);
}

function queueDiscussionPush(notification, serializedMessage, senderAdmin) {
  const targetAdminId = idString(notification.targetAdmin);
  const senderId = idString(senderAdmin);
  if (!targetAdminId || !senderId || targetAdminId === senderId) return;
  const key = `${targetAdminId}:${senderId}`;
  const current = discussionPushBatches.get(key) || { targetAdminId, senderId, senderAdmin, items: [], timer: null };
  current.senderAdmin = senderAdmin;
  current.items.push({ notification, serializedMessage });
  if (current.timer) clearTimeout(current.timer);
  current.timer = setTimeout(() => {
    flushDiscussionPushBatch(key).catch((error) => {
      logger.warn("notification.discussion_grouped_push_failed", { key, error: error.message });
    });
  }, DISCUSSION_PUSH_BATCH_MS);
  discussionPushBatches.set(key, current);
}

async function flushDiscussionPushBatch(key) {
  const batch = discussionPushBatches.get(key);
  if (!batch?.items?.length) return;
  discussionPushBatches.delete(key);

  const latest = batch.items[batch.items.length - 1];
  const latestMessage = latest.serializedMessage;
  const latestNotification = latest.notification;
  const messages = batch.items.map((item) => item.serializedMessage).filter(Boolean);
  const messageIds = messages.map((message) => message.id).filter(Boolean);
  const title = batch.senderAdmin.name || batch.senderAdmin.email || "Admin";
  const body = groupedDiscussionPushBody(messages);
  const latestMeta = discussionPushMeta(latestMessage);
  const image = messages.length > 1
    ? adminAvatarUrl(batch.senderAdmin) || latestMeta.image
    : latestMeta.image || adminAvatarUrl(batch.senderAdmin);
  const groupKey = `discussion:${batch.targetAdminId}:${batch.senderId}`;
  const notificationId = idString(latestNotification);

  await sendPushToAdmins([batch.targetAdminId], {
    ...(latestNotification.toObject?.() || latestNotification),
    title,
    message: body,
    body,
    image,
    ...
    data: {
      ...
      grouped: messages.length > 1 ? "true" : "false",
      senderId: batch.senderId,
      roomId: latestMessage.room,
      type: "discussion_message",
      kind: latestMeta.kind,
      invoiceId: latestMeta.invoiceId,
      invoiceNumber: latestMeta.invoiceNumber,
      image,
      tag: groupKey,
      collapseKey: groupKey,
      groupKey,
...
function reactionMessagePreview(serializedMessage) {
  const text = String(serializedMessage?.message || "").replace(/\s+/g, " ").trim();
  if (text) return text.slice(0, 120);
  if (serializedMessage?.type === "poll") {
    return String(serializedMessage.poll?.question || "poll").replace(/\s+/g, " ").trim().slice(0, 120) || "poll";
  }
  if (serializedMessage?.attachments?.length) {
    const first = serializedMessage.attachments[0];
    const type = String(first?.type || first?.fileType || "").toLowerCase();
    const mime = String(first?.mimeType || "").toLowerCase();
    if (["document", "pdf", "file"].includes(type) || mime === "application/pdf") return first?.fileName || first?.name || "document";
    return first?.fileName || first?.name || "image";
  }
  return "message";
}

function reactionPushBody(serializedMessage, emoji) {
  return `Reacted "${String(emoji || "").trim()}" at "${reactionMessagePreview(serializedMessage)}"`;
}

async function sendReactionPushNotification(serializedMessage, reaction) {
  if (!serializedMessage || reaction?.action !== "added") return null;

  const targetAdminId = idString(reaction.targetAdminId || serializedMessage.senderAdmin);
  const reactorAdminId = idString(reaction.reactorAdminId || reaction.reactorAdmin);
  if (!targetAdminId || !reactorAdminId || targetAdminId === reactorAdminId) {
    logger.info("notification.discussion_reaction_push_skipped", {
      messageId: serializedMessage.id,
      targetAdmin: targetAdminId,
      reactorAdmin: reactorAdminId,
    });
    return null;
  }

  const reactorName = reaction.reactorAdmin?.name || reaction.reactorAdmin?.email || "Admin";
  const body = reactionPushBody(serializedMessage, reaction.emoji);
  const notification = await Notification.create({
    ...
    body,
    icon: "chatbubble",
    senderId: reactorAdminId,
    receiverAdmins: [targetAdminId],
    targetAdmin: targetAdminId,
    ...
    metadata: {
      ...
      reactorAdminId,
      reactorName,
      messagePreview: reactionMessagePreview(serializedMessage),
      roomId: serializedMessage.room,
      channelId: "discussion",
    },
  });

  const notificationId = idString(notification);
  await sendPushToAdmins([targetAdminId], {
  ...
  const unreadCount = await Notification.countDocuments({ targetAdmin: targetAdminId, isRead: false });
  emitNotificationToAdmins([targetAdminId], "notification:new", {
    notification,
    unreadCount,
  });
  emitNotificationToAdmins([targetAdminId], "notification:unread-count", { unreadCount });

  logger.info("notification.discussion_reaction_saved", {
    messageId: serializedMessage.id,
    notificationId,
    targetAdmin: targetAdminId,
    reactorAdmin: reactorAdminId,
...
async function sendDiscussionPushNotifications(notifications, serializedMessage, senderAdmin) {
  ...
  items.forEach((notification) => {
    queueDiscussionPush(notification, serializedMessage, senderAdmin);
  });
}
...
async function publishMessageReaction(message, reaction) {
  if (!message) return;
  if (ioInstance) {
    ioInstance.to(DISCUSSION_ROOM).emit("discussion:reaction-updated", message);
  }
  try {
    await sendReactionPushNotification(message, reaction);
  } catch (error) {
    logger.warn("notification.discussion_reaction_push_failed", {
...
function publishMessagePinUpdated(message, scope = "private", admin = null) {
  if (!ioInstance || !message) return;
  const payload = { message, scope, pinnedBy: admin ? serializeAdmin(admin) : null };
  if (scope === "global") {
    ioInstance.to(DISCUSSION_ROOM).emit("discussion:pin-updated", payload);
    ioInstance.to(DISCUSSION_ROOM).emit("discussion:message-pinned", payload);
    return;
  }
  const targetAdminId = idString(admin);
  if (targetAdminId) {
    emitNotificationToAdmins([targetAdminId], "discussion:pin-updated", payload);
    emitNotificationToAdmins([targetAdminId], "discussion:message-pinned", payload);
  }
}

function publishDiscussionPermissions(settings) {
  if (!ioInstance || !settings) return;
  ioInstance.to(DISCUSSION_ROOM).emit("discussion:permissions-updated", settings);
}
...
async function handleConnected(socket) {
  ...
  socket.on("discussion:join", async (_payload = {}, ack) => {
    ...
    const [count, settings] = await Promise.all([
      unreadCount(socket.admin).catch(() => ({ count: 0 })),
      discussionSettings(socket.admin).catch(() => null),
    ]);
    const payload = onlineUsersPayload();
    socket.emit("discussion:online-users", payload);
    ack?.({ success: true, data: { ...count, settings, onlineUsers: payload.users, onlineCount: payload.onlineCount } });
  });
  ...
  const handleReaction = async (payload = {}, ack) => {
    try {
      const result = await toggleReaction(socket.admin, payload.messageId, payload.emoji);
  ...
  const handleUpdatePermission = async (payload = {}, ack) => {
    try {
      const settings = await updateDiscussionSendPermission(
        socket.admin,
        payload.adminId || payload.targetAdminId,
        payload.canSend !== false && payload.sendingDisabled !== true,
      );
      publishDiscussionPermissions(settings);
  ...
  socket.on("discussion:update-permission", handleUpdatePermission);

  const handlePinMessage = async (payload = {}, ack) => {
    try {
      const scope = payload.scope === "global" || payload.visibleToEveryone ? "global" : "private";
      const message = await pinMessage(socket.admin, payload.messageId, scope);
      publishMessagePinUpdated(message, scope, socket.admin);
      ack?.({ success: true, data: message });
    } catch (error) {
      ack?.({ success: false, message: error.message });
      socket.emit("discussion:error", { message: error.message });
  ...
  const handleUnpinMessage = async (payload = {}, ack) => {
    try {
      const scope = payload.scope === "global" ? "global" : "private";
      const message = await unpinMessage(socket.admin, payload.messageId, scope);
      publishMessagePinUpdated(message, scope, socket.admin);
      ack?.({ success: true, data: message });
    } catch (error) {
      ack?.({ success: false, message: error.message });
  ...
  socket.on("discussion:pin-message", handlePinMessage);
  socket.on("discussion:unpin-message", handleUnpinMessage);
...
module.exports = {
  ...
  publishDiscussionPermissions,
  publishNewMessage,
  publishMessagePinUpdated,

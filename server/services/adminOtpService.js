...
function logDevOtpFallback({ to, otp, secondaryEmail = "", secondaryOtp = "", purpose, challengeId, error }) {
  logger.warn("otp.dev_console_fallback", {
    to,
    purpose,
    challengeId,
    error: error?.message || "",
    message: "Email delivery failed on localhost. Use the OTP printed below to continue login.",
  });
  // eslint-disable-next-line no-console
  console.log("\n========================================");
  // eslint-disable-next-line no-console
  console.log(`[DEV OTP] ${purpose || "login"} → ${to}: ${otp}`);
  if (secondaryEmail && secondaryOtp) {
    // eslint-disable-next-line no-console
    console.log(`[DEV OTP] admin-create-new → ${secondaryEmail}: ${secondaryOtp}`);
  }
  // eslint-disable-next-line no-console
  console.log("========================================\n");
}
  ...
  try {
    ...
    if (isDevelopment()) {
      logDevOtpFallback({
        to: challenge.email,
        otp,
        secondaryEmail: challenge.secondaryEmail,
        secondaryOtp,
        purpose,
        challengeId: String(challenge._id),
        error,

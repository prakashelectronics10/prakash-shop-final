...
function bookingPushBody(booking) {
  return [
    ...
    `Address: ${cleanText(booking.address, "N/A")}`,
    `Pincode: ${cleanText(booking.pincode, "N/A")}`,
    `Landmark: ${cleanText(booking.landmark, "N/A")}`,
...
async function notifyBookingCreated(booking) {
  ...
  const metadata = {
    ...
    pincode: cleanText(booking.pincode),
    landmark: cleanText(booking.landmark),

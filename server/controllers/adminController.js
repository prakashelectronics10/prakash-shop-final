...
function contentImagePublicIds(key, value = {}) {
  ...
  if (key === "footer") {
    return collectPublicIdsFromSources((value.socialLinks || []).map((item) => ({
      iconImagePublicId: item.iconImagePublicId,
      iconImageUrl: item.iconImageUrl,
    })));
  }
...
function assignDisplayOrders(items = []) {
  const used = new Set();
  return items.map((item = {}, index) => {
    let order = Number(item.displayOrder);
    if (!Number.isFinite(order) || order < 1) order = index + 1;
...
async function normalizeProductDisplayOrders(targetProduct) {
  if (!targetProduct?._id) return;
  const items = await Product.find({ isFeatured: Boolean(targetProduct.isFeatured) })
  ...
  await Product.bulkWrite(ordered.map((item, index) => ({
    updateOne: {
      filter: { _id: item._id },
...
function normalizeGalleryContent(value = {}) {
  return {
    ...value,
    items: assignDisplayOrders(value.items || []).map((item = {}, index) => {
      const imageUrl = item.imageUrl || item.src || item.url || "";
      return {
        ...item,
        id: item.id || item._id || `gallery-${Date.now()}-${index}`,
        title: String(item.title || item.label || "").trim(),
        label: String(item.label || item.title || item.alt || `Gallery image ${index + 1}`).trim(),
...
function normalizeAboutContent(value = {}) {
  return {
    ...value,
    reasons: assignDisplayOrders(value.reasons || []).map((item = {}, index) => ({
      ...item,
      id: item.id || item._id || `about-${Date.now()}-${index}`,
      description: String(item.description || item.desc || "").trim(),
...
const createProduct = asyncHandler(async (req, res) => {
  ...
  await normalizeProductDisplayOrders(product);
...
const updateProduct = asyncHandler(async (req, res) => {
  ...
  const nextIds = new Set(collectPublicIdsFromSources(product.imagePublicId, product.imageUrl, product.iconImagePublicId, product.iconImageUrl, product.gallery));
  await deleteCloudinaryImages(collectPublicIdsFromSources(existing.imagePublicId, existing.imageUrl, existing.iconImagePublicId, existing.iconImageUrl, existing.gallery).filter((id) => !nextIds.has(id)));
  await normalizeProductDisplayOrders(product);
  clearSitePayloadCache();
...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  await deleteCloudinaryImages(collectPublicIdsFromSources(product.imagePublicId, product.imageUrl, product.gallery));
  clearSitePayloadCache();
  res.json({ success: true });
});
...
const updateCategory = asyncHandler(async (req, res) => {
  ...
  const nextIds = new Set(collectPublicIdsFromSources(category.imageUrl));
  await deleteCloudinaryImages(collectPublicIdsFromSources(existing.imageUrl).filter((id) => !nextIds.has(id)));
  clearSitePayloadCache();
...
const deleteCategory = asyncHandler(async (req, res) => {
  ...
  if (!category) throw new AppError("Category not found", 404);
  await deleteCloudinaryImages(collectPublicIdsFromSources(category.imageUrl));
  clearSitePayloadCache();
  res.json({ success: true });
});
...
const upsertHero = asyncHandler(async (req, res) => {
  ...
  const nextIds = new Set(collectPublicIdsFromSources(hero.image));
  await deleteCloudinaryImages(collectPublicIdsFromSources(existing?.image).filter((id) => !nextIds.has(id)));
  clearSitePayloadCache();
  res.json({ success: true, data: hero });
});
...
const updateOffer = asyncHandler(async (req, res) => {
  ...
  const nextIds = new Set(collectPublicIdsFromSources(offer.imagePublicId, offer.imageUrl));
  await deleteCloudinaryImages(collectPublicIdsFromSources(existing.imagePublicId, existing.imageUrl).filter((id) => !nextIds.has(id)));
  clearSitePayloadCache();
...
const deleteOffer = asyncHandler(async (req, res) => {
  ...
  await deleteCloudinaryImages(collectPublicIdsFromSources(offer.imagePublicId, offer.imageUrl));
  clearSitePayloadCache();
  res.json({ success: true });
});
...
const upsertContent = asyncHandler(async (req, res) => {
  ...
  const nextValue = normalizeContentValue(key, req.body.value || {});
...
const deleteBooking = asyncHandler(async (req, res) => {
  ...
  await deleteCloudinaryImages(collectPublicIdsFromSources(booking.imagePublicId, booking.imageUrl, booking.images));
  res.json({ success: true });
});

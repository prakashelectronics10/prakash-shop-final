...
exports.getProjectPartSliders = catchAsync(async (req, res) => {
  const sliders = await ProjectPartSlider.find({ isActive: true })
    .select("imageUrl title description displayOrder isActive")
    .sort({ displayOrder: 1, createdAt: -1 })
    .lean();
...
exports.updateSlider = catchAsync(async (req, res) => {
  const { id } = req.params;
  const existing = await ProjectPartSlider.findById(id);
  ...
  const nextIds = new Set(collectPublicIdsFromSources(slider.imagePublicId, slider.imageUrl));
...
exports.deleteSlider = catchAsync(async (req, res) => {
  ...
  await deleteCloudinaryImages(collectPublicIdsFromSources(slider.imagePublicId, slider.imageUrl));

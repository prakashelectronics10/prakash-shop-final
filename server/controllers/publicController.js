...
function bookingProductSource(item = {}) {
  ...
  if (isWiringAccessoriesCategory(item.productCategory)) return "project-part";
  return "";
}
...
const createContactMessage = asyncHandler(async (req, res) => {
  ...
  if (reviewRating) {
    await notifyReviewCreated({ name, phone, email, message, reviewRating }).catch((error) => {
      console.error("Mobile review notification failed:", { error: error.message });
    });
  }

...
function formatCatalogForGemini(products) {
...
function suggestionFromProduct(product, component) {
  return {
    ...
    shortDescription: product.shortDescription || product.description || "Available science project component.",
...
function buildLocalResponse(promptText, imageCount = 0, reason = "") {
  ...
  const note = reason
    ? "\n\nNote: Gemini API se abhi direct response nahi aa paya, isliye maine local project guidance aur shop suggestions generate kiye hain."
    : "";

  return `For this science project, start with a small working prototype and test each module step by step.

Suggested components:
${list}

Basic flow:
1. Connect the controller board and power supply.
2. Add the sensor/input module.
3. Test output using LED, buzzer, relay, or motor.
4. After testing on breadboard, make final wiring clean and safe.${imageCount ? "\n\nI also received your uploaded image(s), but advanced image analysis needs Gemini connection." : ""}${note}`;
}
...
async function buildProductSuggestions(promptText, aiText, availableProducts) {
...
exports.chatWithScienceAI = catchAsync(async (req, res) => {
  const { message, imageBase64, imageMimeType = "image/jpeg", images = [], conversationHistory = [] } = req.body;
  ...
  if (!apiKey) {
    ...
    const suggestions = await buildProductSuggestions(message, aiResponse);
    return res.json({
      success: true,
      data: {
        response: aiResponse,
  ...
  try {
    availableCatalogProducts = await fetchAvailableCatalogProducts();
  } catch (_error) {
    availableCatalogProducts = [];
  }

  const systemContext = `You are Science AI, a helpful assistant specialized in science projects and electronics components.
You help students and hobbyists with project ideas, electronic component selection, circuit design, image analysis, and troubleshooting.
Keep continuity with previous messages. Be practical, educational, and safety-aware.
...
When the user types a product name, normalize spelling and identify the likely component/product.
When the user asks about a project, include a short required components list in your answer.
Use the available catalog below as the only source of shop/product availability. If a catalog product name/model is a close match to the image or typed product name, mention that exact catalog product name in your answer. If nothing matches, say that you cannot confirm a matching available catalog item.

${formatCatalogForGemini(availableCatalogProducts)}`;
  ...
  currentParts.push({ text: message || "Please analyze these images and identify useful science project components." });
  ...
  if (!geminiResponse?.ok && geminiError && (shouldUseLocalFallback(geminiError) || isRateLimitError(geminiError))) {
    ...
    const suggestions = await buildProductSuggestions(message, aiResponse, availableCatalogProducts);

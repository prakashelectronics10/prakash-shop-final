...
exports.getProjectParts = catchAsync(async (req, res) => {
  ...
  const parts = await ProjectPart.find(query)
    .select("name slug description shortDescription category subCategory mrp discountPercent price stock availability imageUrl tags isActive isFeatured displayOrder createdAt")
...
exports.createProjectPart = catchAsync(async (req, res) => {
  ...
  const part = await ProjectPart.create({
    ...
    description,
    shortDescription,
    category: category || "Wiring Products",
...
exports.updateProjectPart = catchAsync(async (req, res) => {
  ...
  if (Object.prototype.hasOwnProperty.call(req.body, "subCategory")) {
    updateData.subCategory = String(req.body.subCategory || "").trim();
  }
  if (Object.prototype.hasOwnProperty.call(req.body, "category")) {
    updateData.category = String(req.body.category || "").trim() || "Wiring Products";
  }
...
// Get categories for project parts / wiring accessories
...
// Get sub-categories (brands) optionally filtered by category
exports.getProjectPartSubCategories = catchAsync(async (req, res) => {
  const category = String(req.query.category || "").trim();
  const match = { isActive: true, subCategory: { $nin: [null, ""] } };
  if (category) match.category = category;

  const subCategories = (await ProjectPart.distinct("subCategory", match))
    .map((item) => String(item || "").trim())
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b));
...
// Taxonomy for admin/public filters: categories + brands grouped by category
exports.getProjectPartTaxonomy = catchAsync(async (req, res) => {
  const parts = await ProjectPart.find({ isActive: true })
  ...
  parts.forEach((part) => {
    ...
    const subCategory = String(part.subCategory || "").trim();
    if (category) categorySet.add(category);
    if (!category) return;
    if (!byCategory[category]) byCategory[category] = new Set();
    if (subCategory) {
      byCategory[category].add(subCategory);
  ...
  res.set("Cache-Control", "public, max-age=120, stale-while-revalidate=400");
  res.json({
    success: true,
    data: {
      categories,
      subCategories: [...allSubCategories].sort((a, b) => a.localeCompare(b)),

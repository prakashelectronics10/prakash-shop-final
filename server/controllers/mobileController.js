...
const savePushToken = asyncHandler(async (req, res) => {
  ...
  await Admin.updateOne(
    { _id: req.admin._id },
    {
      $push: {
        pushTokens: {
          $each: [{ token, deviceId, platform, lastSeenAt: new Date(), createdAt: new Date() }],
          $slice: -5,
        },
        expoPushTokens: {
          $each: [{ token, deviceId, platform, lastSeenAt: new Date(), createdAt: new Date() }],
  ...
  const updatedAdmin = await Admin.findById(req.admin._id).select("expoPushTokens").lean();
  ...
  res.json({ success: true, expoPushTokenCount: updatedAdmin?.expoPushTokens?.length || 0 });
});

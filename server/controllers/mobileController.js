...
function normalizeAppSettings(value = {}) {
  return {
    brandName: String(value.brandName || "Prakash Electronics").trim() || "Prakash Electronics",
    logoUrl: String(value.logoUrl || value.logo?.url || "").trim(),
    logoPublicId: String(value.logoPublicId || value.logo?.publicId || "").trim(),
    updatedAt: value.updatedAt || null,
  };
}

async function getAppSettingsDoc() {
  const doc = await SiteContent.findOne({ key: "appSettings" }).lean();
  return normalizeAppSettings(doc?.value || {});
}
...
const updateAppLogo = asyncHandler(async (req, res) => {
  if (!req.file) throw new AppError("App logo image is required", 400);

  const existing = await SiteContent.findOne({ key: "appSettings" }).lean();
  const previous = normalizeAppSettings(existing?.value || {});
  const uploaded = await uploadBuffer(req.file.buffer, {
    folder: "prakash-electronics/app-settings/logo",
  });
  const nextValue = normalizeAppSettings({
    ...previous,
  ...
  await SiteContent.findOneAndUpdate(
    { key: "appSettings" },
    { key: "appSettings", value: nextValue },
    { new: true, upsert: true, runValidators: true },
  );
  clearSitePayloadCache();

  const previousPublicIds = collectPublicIdsFromSources(previous.logoPublicId, previous.logoUrl)
    .filter((publicId) => publicId !== uploaded.public_id);
  if (previousPublicIds.length) {
    deleteImagesStrict(previousPublicIds).catch((error) => {
      console.warn("Previous app logo cleanup failed:", { error: error.message });
    });
  }

  res.status(201).json({ success: true, data: nextValue });
});
...
const savePushToken = asyncHandler(async (req, res) => {
  ...
  await Admin.updateOne(
    ...
    {
      $pull: {
        pushTokens: deviceId ? { $or: [{ token }, { deviceId }] } : { token },
      },
    },
  );
  await Admin.updateOne(
    ...
    {
      $pull: {
        expoPushTokens: deviceId ? { $or: [{ token }, { deviceId }] } : { token },
      },
    },
  );
  if (nativePushToken) {
    await Admin.updateOne(
      ...
      {
        $pull: {
          fcmPushTokens: deviceId ? { $or: [{ token: nativePushToken }, { deviceId }] } : { token: nativePushToken },
        },
      },
    );
  }
...
const listNotifications = asyncHandler(async (req, res) => {
  ...
  const [items, total, unreadCount] = await Promise.all([
    Notification.find(filter)
      ...
      .populate("senderId", "name email role tag avatarUrl avatarPublicId updatedAt")
      .populate("requestedAdmin", "name email role tag adminAndroidAppAccess avatarUrl avatarPublicId updatedAt")
      .sort({ createdAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean(),
...
const deleteNotification = asyncHandler(async (req, res) => {
  ...
  const unreadCount = await Notification.countDocuments({ targetAdmin: req.admin._id, isRead: false });
  emitNotificationToAdmins([req.admin._id], "notification:deleted", { notificationIds: [String(notification._id)], unreadCount });
  emitNotificationToAdmins([req.admin._id], "notification:unread-count", { unreadCount });
  res.json({ success: true, data: { deletedIds: [String(notification._id)], unreadCount } });
});

const deleteNotifications = asyncHandler(async (req, res) => {
  const ids = [...new Set((req.body?.ids || req.body?.notificationIds || [])
    .map((id) => String(id || "").trim())
    .filter(Boolean))];
  if (!ids.length) throw new AppError("Select at least one notification", 400);
  ...
  const unreadCount = await Notification.countDocuments({ targetAdmin: req.admin._id, isRead: false });
  emitNotificationToAdmins([req.admin._id], "notification:deleted", { notificationIds: ids, unreadCount });
  emitNotificationToAdmins([req.admin._id], "notification:unread-count", { unreadCount });
  res.json({ success: true, data: { deletedIds: ids, deletedCount: result.deletedCount || 0, unreadCount } });
});
